...
// 그림 아래쪽 25% 지점부터 잔디·건물 그림이 있어 글이 겹치지 않도록 여백을 넉넉히 둔다
export const DEFAULT_BOTTOM = 30;
export const DEFAULT_SCALE = 0.95;
...
export function posterBlocks(data, center) {
...
export function applyBlocks(data, center) {
  return [
    { type: 'centertitle', text: `${center} 어린이집 참관 신청서` },
    ...
    { type: 'sign', date: '        년      월      일', role: '신청인', name: '                 ' },
  ];
}
...
export function buildVisitDoc(data, basic) {
  ...
  return [
    ...
    ...posterBlocks(data, center),
    { type: 'heading', text: '3. 참관 신청서 (양식)' },
    ...applyBlocks(data, center),
    { type: 'heading', text: '4. 참관 기록' },
    ...logBlocks(data, center),
    { type: 'heading', text: '5. 심사 점검 항목' },
    {
      type: 'table',
      head: ['점검 항목', '이 문서에서 확인할 수 있는 곳'],
      widths: ['58%', '42%'],
      rows: [
        [CHECKPOINTS[0], '1. 참관 상시 운영 계획 (운영 원칙)'],
        [CHECKPOINTS[1], '2. 게시용 참관 안내문 (현관·게시판 게시)'],
        [CHECKPOINTS[2], '2. 게시용 참관 안내문 (자격·시기·방법)'],
      ],
    },
    { type: 'note', text: '※ 이 항목은 서류 제출이 아니라 현장확인 항목입니다. 참관 안내문 게시 여부를 눈으로 확인하고, 상시 운영 여부는 원장 면담으로 확인합니다.' },
  ];
}
