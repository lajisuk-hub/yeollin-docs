...
const DOC_STYLE = {
  ...
  head: { char: 17, para: 29 }, // 굵게 · 왼쪽 초록 세로띠 · 다음 문단과 붙어 다님
...
function picPara(itemId, orgW, orgH, curW, curH, tight) {
  ...
  return (
    `<hp:p id="${nid()}" paraPrIDRef="${tight ? 30 : 25}" styleIDRef="0" pageBreak="0" columnBreak="0" merged="0">` +
    `<hp:run charPrIDRef="${tight ? 2 : 15}">${pic}</hp:run></hp:p>`
  );
}
...
// 1픽셀 = 75 HWPUNIT (96dpi). 예전 값 48은 그림의 오른쪽·아래가 잘려 2/3만 보였다 (2026-09-19 실측)
const PX = 75;

// ─────────────────────────────────────────────────────────────
// 화면 블록을 그림으로 찍기 (2026-09-19)
// 표·회의록 상자·그래프·서식 그림 안내문은 한글에서 다시 그리면 PDF와 모양이 달라진다.
// 그래서 화면에 그리는 것과 똑같은 <Block>을 보이지 않는 곳에 그려 html2canvas로 찍어 넣는다.
// 본문 폭(170mm)으로 그리므로 PDF와 줄바꿈까지 같다. 너무 긴 그림은 글자가 없는 줄에서 잘라 여러 장으로.
const SNAP_W = 643;          // 170mm @96dpi — PDF 본문 폭
// 그림 한 장의 세로/가로 한계. 그림은 쪽을 넘어가며 나뉘지 못하므로 크면 앞 쪽에 큰 공백이 남는다.
// 그래서 글자 없는 줄에서 잘게 잘라(약 75mm) 이어 붙인다 — 문단 간격 0인 모양(21)으로 넣어 이음매가 안 보인다.
const SNAP_MAX_RATIO = 0.5;
const SNAP_WHOLE_RATIO = 1.42; // 통째로 넣는 그림(서식·사진)의 한계 — 한글 한 쪽에 들어가는 높이

const wait = (ms) => new Promise((r) => setTimeout(r, ms));

// 글자가 가장 적은 가로줄을 찾아 그 자리에서 자른다 (표 줄 사이·문단 사이)
function sliceCanvas(canvas, whole) {
  const W = canvas.width;
  const H = canvas.height;
  const maxH = Math.floor(W * (whole ? SNAP_WHOLE_RATIO : SNAP_MAX_RATIO));
  if (H <= maxH) return [canvas];
  const ctx = canvas.getContext('2d', { willReadFrequently: true });
  const ink = (y) => {
    const d = ctx.getImageData(0, y, W, 1).data;
    let n = 0;
    for (let i = 0; i < d.length; i += 16) if (d[i] + d[i + 1] + d[i + 2] < 600) n += 1; // 4픽셀마다
    return n;
  };
  const cuts = [0];
  while (H - cuts[cuts.length - 1] > maxH) {
    const start = cuts[cuts.length - 1];
    const hi = start + maxH;
    const lo = start + Math.floor(maxH * 0.72);
    let best = hi;
    let bestInk = Infinity;
    for (let y = hi; y >= lo; y -= 1) {
      const n = ink(y);
      if (n < bestInk) { bestInk = n; best = y; if (n === 0) break; }
    }
    cuts.push(best);
  }
  cuts.push(H);
  ...
  for (let i = 0; i < cuts.length - 1; i += 1) {
    ...
    g.fillStyle = '#fff';
    g.fillRect(0, 0, W, h);
    g.drawImage(canvas, 0, cuts[i], W, h, 0, 0, W, h);
    out.push(c);
  }
  return out;
}

async function expandSnaps(blocks, onProgress) {
  const snaps = blocks.filter((b) => b && b.kind === 'snap' && b.block);
  if (!snaps.length) return blocks;
  if (onProgress) onProgress('표와 서식을 화면 모양 그대로 옮기는 중입니다...');

  const [React, { createRoot }, { default: Block }, { default: html2canvas }] = await Promise.all([
    import('react'), import('react-dom/client'), import('../app/NewBlocks'), import('html2canvas'),
  ]);

  const host = document.createElement('div');
  host.className = 'doc-page hwpx-snap-host';
  host.setAttribute('aria-hidden', 'true');
  host.style.cssText = `position:fixed;left:-20000px;top:0;width:${SNAP_W}px;max-width:none;min-height:0;margin:0;padding:0;box-shadow:none;background:#fff;`;
  ...
  try {
    root.render(
      React.createElement(React.Fragment, null,
        snaps.map((s, i) => React.createElement('div', { key: i, 'data-snap': i, style: { padding: '2px', background: '#fff', overflow: 'hidden' } },
          React.createElement(Block, { b: s.block }))))
    );
    await wait(60);
    // 그림이 다 뜰 때까지 (서식 그림·올린 사진)
    await Promise.all([...host.querySelectorAll('img')].map((img) => (img.complete ? null : new Promise((r) => { img.onload = r; img.onerror = r; }))));
    if (document.fonts?.ready) await document.fonts.ready;
    await wait(350); // 서식 그림 위 글자 크기 맞춤(useFitText)이 끝나기를 기다린다
    // html2canvas는 cqw 단위를 모른다 → 계산된 px로 바꿔 둔다
    host.querySelectorAll('[style*="cqw"]').forEach((el) => { el.style.fontSize = getComputedStyle(el).fontSize; });

    const nodes = [...host.querySelectorAll('[data-snap]')];
    for (let i = 0; i < nodes.length; i += 1) {
      if (onProgress) onProgress(`표와 서식을 옮기는 중입니다... (${i + 1}/${nodes.length})`);
      const el = nodes[i];
      if (el.offsetHeight < 8) { results.set(snaps[i], []); continue; } // 내용이 없는 블록
      // html2canvas는 찍을 때마다 문서 전체를 복제한다 → 화면의 미리보기(사진 수십 장)는 빼고 복제해야 빠르다
      const canvas = await html2canvas(el, {
        scale: 2, backgroundColor: '#ffffff', useCORS: true, logging: false,
        ignoreElements: (node) => node.parentElement === document.body && node !== host,
      });
      results.set(snaps[i], sliceCanvas(canvas, snaps[i].whole).map((c) => c.toDataURL('image/jpeg', 0.92)));
    }
  } finally {
    root.unmount();
    host.remove();
  ...
  const out = [];
  blocks.forEach((b) => {
    if (!(b && b.kind === 'snap')) { out.push(b); return; }
    const parts = results.get(b) || [];
    parts.forEach((src) => out.push({ kind: 'image', src, ratio: 1, tight: true }));
    if (parts.length) out.push({ kind: 'gap' });
  });
  return out;
}
...
export async function buildDocHwpx({ blocks, onProgress }) {
  ...
  // ── 표·상자·그래프는 화면(PDF)과 똑같이 그림으로 (kind:'snap') ──
  blocks = await expandSnaps(blocks || [], onProgress);
  ...
  (blocks || []).forEach((b) => {
    ...
    if (b.kind === 'gap') { xml += para('', DOC_STYLE.note); return; }
    ...
    if (b.kind === 'image') {
      ...
      if (!p) return;
      if (pendingBreak) { xml += para('', DOC_STYLE.body, { pageBreak: true }); pendingBreak = false; }
      xml += picPara(p.id, p.orgW, p.orgH, p.curW, p.curH, b.tight);
      return;
    }
...
export function downloadBlob(blob, filename) {
  ...
  // 만드는 데 몇 초 걸린 저장은 브라우저가 조용히 막기도 한다 → 직접 누르는 단추를 1분 동안 띄워 둔다
  document.querySelectorAll('.hwpx-save-toast').forEach((el) => el.remove());
  const box = document.createElement('div');
  box.className = 'hwpx-save-toast';
  box.style.cssText = 'position:fixed;left:50%;bottom:24px;transform:translateX(-50%);z-index:9999;background:#1f3d35;color:#fff;padding:12px 16px;border-radius:12px;box-shadow:0 6px 24px rgba(0,0,0,.25);font-size:14px;display:flex;gap:12px;align-items:center;max-width:92vw;';
  const msg = document.createElement('span');
  msg.textContent = '한글 파일이 준비됐습니다. 저장이 안 됐으면 →';
  const link = document.createElement('a');
  link.href = url;
  link.download = filename;
  link.textContent = '📥 파일 저장하기';
  link.style.cssText = 'background:#fff;color:#1f3d35;font-weight:700;padding:7px 12px;border-radius:8px;text-decoration:none;white-space:nowrap;';
  const close = document.createElement('button');
  close.textContent = '✕';
  close.style.cssText = 'background:none;border:0;color:#fff;font-size:15px;cursor:pointer;';
  const done = () => { box.remove(); URL.revokeObjectURL(url); };
  close.onclick = done;
