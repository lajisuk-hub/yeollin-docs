...
// 기관마다 정리해야 하는 기간이 달라(25년 3·4분기 ~ 26년 1·2분기 등)
// 아래 분기 중에서 원장님이 직접 골라 차수를 만든다. (2026-08-19 추가)
const Q_WHEN = {
  1: (y) => `${y}년 3월`,
  2: (y) => `${y}년 6월 ~ 8월`,
  3: (y) => `${y}년 9월 ~ 11월`,
  4: (y) => `${y}년 12월 ~ ${Number(y) + 1}년 2월`,
};
const Q_NOTE = { 1: '1분기는 3월 안에 열어야 인정됩니다' };

// 분기 하나의 정보 — key는 '2025-4' 처럼 적는다
export function quarterInfo(key) {
  const [year, n] = String(key).split('-');
  const qn = Number(n);
  const out = {
    key: `${year}-${qn}`,
    year,
    qn,
    quarter: `${year}년 ${qn}분기`,
    when: (Q_WHEN[qn] || Q_WHEN[1])(year),
  };
  if (Q_NOTE[qn]) out.note = Q_NOTE[qn];
  return out;
}

// 화면에 버튼으로 보여 줄 분기 (넉넉히)
export const QUARTER_KEYS = ['2025-3', '2025-4', '2026-1', '2026-2', '2026-3', '2026-4'];
export const QUARTER_OPTIONS = QUARTER_KEYS.map(quarterInfo);

// 아무것도 고르지 않았을 때 — 심사는 직전 1년을 보므로 25년 4분기 ~ 26년 3분기
export const DEFAULT_PICKS = ['2025-4', '2026-1', '2026-2', '2026-3'];

const qOrder = (k) => {
  const [y, n] = String(k).split('-');
  return Number(y) * 10 + Number(n);
};

// 고른 분기를 시간 순으로 정리 (중복·엉뚱한 값 제거)
export const sortPicks = (picks) =>
  Array.from(new Set((picks || []).filter((k) => QUARTER_KEYS.includes(k)))).sort((a, b) => qOrder(a) - qOrder(b));

// 고른 분기 → 1차·2차… 차수 목록
export function makeMeetings(picks) {
  const keys = sortPicks(picks);
  const use = keys.length ? keys : DEFAULT_PICKS;
  return use.map((k, i) => ({ no: `${i + 1}차`, ...quarterInfo(k) }));
}

// 기본 차수 (고른 것이 없을 때 쓰는 값)
export const MEETINGS = makeMeetings(DEFAULT_PICKS);

// 저장된 내용에서 그 원의 차수 목록 꺼내기
export const meetingsOf = (data) => makeMeetings(data?.picks);

// 고른 분기에 걸쳐 있는 연도 (회칙·위원 명단은 연도별로 만든다)
export const yearsOf = (data) => Array.from(new Set(meetingsOf(data).map((m) => m.year)));

// 원장님 서식 제목 표기 — 예) 2025년도 4/4분기
export const qLabel = (i, list = MEETINGS) => `${list[i].year}년도 ${list[i].qn}/4분기`;

// 결과 안내문 제목 표기 — 예) 2026-2월  (실제로 회의를 연 달 기준)
export function noticeMonthLabel(m, i, list = MEETINGS) {
  if (!m?.date) return qLabel(i, list);
...
export const emptyMeeting = () => ({
  ...
  // 개최 공지문 — 일시·장소·안건은 표로 들어가고, 인사말과 맺음말만 글로
  ...
  // 결과보고서 — 안건별 표로 들어간다
  resultIntro: '', resultItems: [], resultClosing: '', resultFeedback: '',
  feature: '', featureFeedback: '',          // 그 분기 운영의 특징 정리
  photos: [],
});

export const emptyData = () => ({
  picks: [...DEFAULT_PICKS],            // 정리할 분기 (원장님이 고른다)
  members: { 2025: [], 2026: [] },
  rules: { 2025: { text: '', feedback: '' }, 2026: { text: '', feedback: '' } },
  // 원장님이 가진 서식을 한 번만 붙여넣으면 모든 차수를 그 틀로 만든다
  samples: { rules: '', notice: '', minutes: '', result: '' },
  meetings: MEETINGS.map(() => emptyMeeting()),
});

// 고른 분기를 바꿔도 이미 적어 둔 내용은 그 분기를 따라간다
export function applyPicks(data, picks, fresh = emptyMeeting) {
  const before = meetingsOf(data);
  const keep = {};
  before.forEach((mi, i) => { keep[mi.key] = data?.meetings?.[i]; });
  const after = makeMeetings(picks);
  const years = Array.from(new Set(after.map((x) => x.year)));
  const members = { ...(data?.members || {}) };
  const rules = { ...(data?.rules || {}) };
  years.forEach((y) => {
    if (!members[y]) members[y] = [];
    if (!rules[y]) rules[y] = { text: '', feedback: '' };
  });
  return {
    ...data,
    ...
    members,
    rules,
    meetings: after.map((mi) => keep[mi.key] || fresh()),
  };
}
...
export function meetingHasContent(m) {
  return !!(m && (m.date || m.agenda || m.noticeGreeting || m.memo || m.discussion
    || (m.resultItems || []).length || (m.photos || []).length));
}

// 그 차수에 실제로 참석한 위원 (그 해의 명단에서)
export function membersOf(data, i) {
  const info = meetingsOf(data)[i];
  return (data?.members?.[info?.year] || []).filter((x) => x.name?.trim());
}
...
export function meetingDone(m) {
  return !!(m?.date && m?.agenda && m?.noticeGreeting && m?.discussion && (m?.resultItems || []).length);
}

// 회의록 마지막 줄 — 원장님 서식은 다음 분기 회의 시기를 예고하고 맺는다
export function closingText(i, list = MEETINGS) {
  const cur = list[i].qn;
...
export function defaultAgenda(i, list = MEETINGS) {
  const { year, qn } = list[i];
  const y = Number(year);
  return {
    1: [
      '운영위원 위촉 및 위원장 선출',
...
export function defaultMemo(i, list = MEETINGS) {
  const { year, qn } = list[i];
  const y = Number(year);
  return {
    1: [
...
// 회칙 글자 다듬기 — PDF·한글에서 글자만 뽑으면 줄바꿈이 사라져 한 덩어리로 붙는다.
// 장 · 조 · 항(①②③) · 호(1. 2. 3.) · 부칙 앞에서 줄을 나눠 문서 모양이 살아나게 한다.
// 글자는 바꾸지 않고 줄만 넣는다. (이미 줄이 나뉘어 있으면 그대로 둔다)
export function normalizeRules(text) {
  let s = String(text || '').replace(/\r\n?/g, '\n').trim();
  if (!s) return '';
  // 올린 파일 앞에 「구성 계획」 쪽이 붙어 있으면 뺀다 — 구성계획은 앱이 1번 항목으로 따로 만들어 넣으므로
  // 회칙 자리에 또 들어가면 굵은 글씨 한 덩어리로 엉킨다. 회칙 제목(○○ 운영위원회 회칙)부터 시작하게 한다.
  const firstBody = s.search(/제\s*1\s*(장|조\s*\()/);
  if (firstBody > 0) {
    const head = s.slice(0, firstBody);
    const titles = [...head.matchAll(/(\S+\s+)?운영위원회\s*회\s*칙/g)];
    const last = titles[titles.length - 1];
    if (last && last.index > 0) s = s.slice(last.index);
  }
  s = s
    // 글자만 뽑으면 「제 25 조」「2024 년 3 월」처럼 숫자 앞뒤가 벌어진다 — 빈칸만 붙인다 (글자는 그대로)
    .replace(/제\s+(?=\d)/g, '제')
    .replace(/(\d)\s+(?=[년월일조장항호명인회])/g, '$1')
    .replace(/(^|\s)부\s+칙(?=\s|$)/g, '$1부칙')
    .replace(/\(\s+/g, '(').replace(/\s+\)/g, ')')
    .replace(/\s*(제\s*\d+\s*장)/g, '\n\n$1')
    .replace(/\s*(제\s*\d+\s*조\s*\()/g, '\n\n$1')
    .replace(/\s*(부\s*칙)(?=\s|$)/g, '\n\n$1')
    .replace(/(회\s*칙)\s+(제정\s*\d)/, '$1\n$2')
    .replace(/\s*([①-⑳])/g, '\n$1')
    .replace(/\n{3,}/g, '\n\n');
  // 호(1. 2. 3.)는 번호가 1이거나 바로 앞 번호+1로 이어질 때만 줄을 나눈다 — 날짜(2024. 03. 01.)의 조각은 건드리지 않는다
  let last = 0;
  s = s.replace(/(\S+)(\s+)([1-9]\d?)\.(?=\s)/g, (all, prev, sp, num) => {
    const n = Number(num);
    if (/\d\.$/.test(prev) || !(n === 1 || n === last + 1)) return all;
    last = n;
    return `${prev}\n${num}.`;
  });
  s = s
...
function meetingBlocks(data, i, opts = {}) {
  ...
  if (!opts.noHead) {
    b.push({ type: 'sessionhead', text: `${info.no} · ${qLabel(i, list)} : ${whenText(m) || '일시 미입력'}` });
  }

  // 회칙·구성계획은 그 해 첫 차수 안에만 (연도별 1회)
  if (opts.withRules) {
    const center = opts.center || '○○어린이집';
    b.push({ type: 'subheading', text: `${info.year}년도 ${center} 운영위원회 구성계획` });
    planSection(data, info.year, center).forEach((x) => b.push(x));
    b.push({ type: 'subheading', text: `${info.year}년 운영위원회 회칙` });
    b.push({ type: 'rulesdoc', text: normalizeRules(data?.rules?.[info.year]?.text?.trim() || defaultRulesText(info.year, center)) });
    b.push({ type: 'subheading', text: `${info.qn}분기 운영위 내용` });
  }
  ...
  b.push({
    type: 'noticedoc',
    title: `${qLabel(i, list)} 어린이집 운영위원회 개최 안내`,
    greeting: m.noticeGreeting || '',
    rows: [
      ['일 시', whenText(m)],
      ['장 소', m.place || ''],
      ['안 건', agenda.length ? agenda.map((t, n) => `${n + 1}. ${t}`).join('\n') : ''],
      ['참석 대상', (members || []).map((x) => x.role).filter((v, n, a) => a.indexOf(v) === n).join(', ')],
    ],
    closing: m.noticeClosing || '',
    center: opts.center || '',
  });

  // ② 회의록 (원장님 서식: 표 → 회의순서 → 토의 및 의결사항 → 서명)
  b.push({ type: 'subheading', text: `운영위 회의록 (${qLabel(i, list)})` });
  b.push({
    type: 'minutesdoc',
    title: `${qLabel(i, list)} 운영위원회 회의록`,
    info: [
      ['일 시', whenText(m)],
      ['장 소', m.place || ''],
      ['간 사', m.secretary || ''],
    ...
    order: m.order || '',
    discussion: m.discussion || m.memo || '',
    closing: closingText(i, list),
    signRows: signRows(members),
  });

  // ③ 회의결과 보고서(안내문)
  b.push({ type: 'subheading', text: `운영위 결과보고서 (${noticeMonthLabel(m, i, list)})` });
  b.push({
    type: 'resultdoc',
    title: `${noticeMonthLabel(m, i, list)} 운영위원회 회의결과 안내문`,
    intro: m.resultIntro || '',
    items: m.resultItems || [],
    closing: m.resultClosing || '',
    center: opts.center || '',
  });
...
// 구성계획·회칙은 그 해에 한 번만 넣는다 (그 회계연도의 첫 차수에만)
export const isFirstOfYear = (i, list = MEETINGS) => list.findIndex((x) => x.year === list[i].year) === i;

// 그 해 구성계획·회칙이 어느 차수 문서에 들어 있는지 (안내 문구용)
export const yearDocAt = (i, list = MEETINGS) => list[list.findIndex((x) => x.year === list[i].year)];

// 한 차수만 미리 보기 — 원장님이 정한 구성 순서
//   그 해 첫 차수 : 1. 구성계획  2. 회칙  3. 그 분기 운영위 내용
//   그 다음 차수  : 1. 그 분기 운영위 내용  (회칙·구성계획은 연도별 한 번이면 됨)
export function buildOneMeetingDoc(data, i, basic) {
  ...
  const blocks = [
    ...
    { type: 'lead', text: `${info.no} · ${qLabel(i, list)} (${whenText(data.meetings[i]) || '일시 미입력'})` },
  ];

  let n = 1;
  if (isFirstOfYear(i, list)) {
    blocks.push({ type: 'heading', text: `${n++}. ${y}년도 ${center} 운영위원회 구성계획` });
    planSection(data, y, center).forEach((x) => blocks.push(x));
    blocks.push({ type: 'heading', text: `${n++}. ${y}년 운영위원회 회칙` });
    blocks.push({ type: 'rulesdoc', text: normalizeRules(data?.rules?.[y]?.text?.trim() || defaultRulesText(y, center)) });
  } else {
    const first = yearDocAt(i, list);
    blocks.push({ type: 'note', text: `※ ${y}년도 운영위원회 구성계획과 회칙은 ${first.no}(${first.quarter}) 문서에 들어 있습니다. (연도별 1회)` });
  }

  blocks.push({ type: 'heading', text: `${n}. ${info.qn}분기 운영위 내용` });
  meetingBlocks(data, i, { noHead: true, center }).forEach((x) => blocks.push(x));
  return blocks;
}
...
export function buildCommitteeDoc(data, basic) {
  const center = basic?.centerName?.trim() || '○○어린이집';
  // 차수별 정리본을 그대로 이어 붙인다 (새로 짜맞추지 않음).
  // 차수마다 새 쪽에서 시작하고, 쪽 위아래 여백은 인쇄 틀(PrintSheet)이 모든 쪽에 넣어 준다.
  const blocks = [];
  let first = true;
  meetingsOf(data).forEach((info, i) => {
    if (!meetingHasContent(data.meetings[i])) return;
    if (!first) blocks.push({ type: 'pagebreak' });
    first = false;
    buildOneMeetingDoc(data, i, basic).forEach((x) => blocks.push(x));
  });
  if (first) {
    blocks.push({ type: 'title', text: `${center} 운영위회의 자료` });
...
// 화면 블록 → 한글(hwpx) 내용
// 2026-08-18: 표는 진짜 표로, 사진은 진짜 사진으로, 서류마다 새 쪽에서 시작한다.
export function toHwpxBlocks(blocks) {
  const out = [];
  // 새 쪽에서 시작하기 — 바로 앞이 소제목이면 소제목까지 데리고 넘어간다
  // (안 그러면 "운영위 회의록" 같은 소제목만 앞 쪽 맨 아래에 덩그러니 남는다)
  const breakPage = () => {
    const last = out[out.length - 1];
    if (last && last.kind === 'head') out.splice(out.length - 1, 0, { kind: 'pagebreak' });
    else out.push({ kind: 'pagebreak' });
  };
  // 이름칸 + 내용칸 두 칸짜리 표 (일시·장소·참석자 …)
  const kvTable = (rows) =>
    out.push({
      kind: 'table',
      labelFirst: true,
      widths: [22, 78],
      rows: (rows || []).map(([k, v]) => [k, String(v == null ? '' : v)]),
    });

  blocks.forEach((b) => {
    if (b.type === 'title') out.push({ kind: 'title', text: b.text });
    else if (b.type === 'sessionhead') {
      breakPage();
      out.push({ kind: 'title', text: b.text });
    }
    else if (b.type === 'heading' || b.type === 'subheading') out.push({ kind: 'head', text: b.text });
    else if (b.type === 'para') out.push({ kind: 'body', text: b.text });
    else if (b.type === 'note') out.push({ kind: 'note', text: b.text });
    else if (b.type === 'kv') kvTable(b.rows);
    else if (b.type === 'table') {
      out.push({ kind: 'table', head: b.head || [], rows: b.rows || [], widths: b.widths });
    }
    else if (b.type === 'rulesdoc') String(b.text || '').split('\n').forEach((t) => {
      const s = t.trim();
      if (!s) return;
      out.push({ kind: /^제\s*\d+\s*장|^부칙/.test(s) ? 'head' : 'body', text: s });
    });
    else if (b.type === 'noticedoc') {
      breakPage();
      out.push({ kind: 'title', text: b.title });
      if (b.greeting) out.push({ kind: 'body', text: b.greeting });
      kvTable(b.rows);
      if (b.closing) out.push({ kind: 'body', text: b.closing });
      if (b.center) { out.push({ kind: 'body', text: '' }); out.push({ kind: 'sign', text: b.center }); }
    }
    else if (b.type === 'minutesdoc') {
      breakPage();
      out.push({ kind: 'title', text: b.title });
      kvTable(b.info);
      if (b.order) { out.push({ kind: 'head', text: '회의순서' }); out.push({ kind: 'body', text: b.order }); }
      out.push({ kind: 'head', text: '토의 및 의결사항' });
      out.push({ kind: 'body', text: b.discussion || '' });
      if (b.closing) out.push({ kind: 'body', text: b.closing });
      if ((b.signRows || []).length) {
        out.push({ kind: 'body', text: '' });
        out.push({
          kind: 'table',
          head: ['직위', '성명', '서명'],
          widths: [34, 33, 33],
          rows: b.signRows.map((r) => [r[0] || '', r[1] || '', r[2] || '']),
        });
      }
    }
      ...
      if ((b.items || []).length) {
        out.push({
          ...
          head: ['안건', '결과'],
          widths: [28, 72],
          rows: b.items.map((it, n) => [`${n + 1}) ${it.title || ''}`, it.body || '']),
        });
      }
      if (b.closing) out.push({ kind: 'body', text: b.closing });
      if (b.center) { out.push({ kind: 'body', text: '' }); out.push({ kind: 'sign', text: b.center }); }
      ...
      breakPage();
      out.push({ kind: 'head', text: b.caption || '회의 사진' });
      items.forEach((src) => out.push({ kind: 'image', src, ratio: 0.62 }));
    }
  });
  return out;
}
