...
export function toHwpxBlocks(blocks) {
  ...
  blocks.forEach((b) => {
    if (b.type === 'title') out.push({ kind: 'title', text: b.text });
      ...
      (b.rows || []).forEach(([k, v]) => out.push({ kind: 'body', text: `${k} : ${String(v || '').replace(/\n/g, ' / ')}` }));
      if (b.closing) out.push({ kind: 'body', text: b.closing });
      if (b.center) out.push({ kind: 'note', text: b.center });
    }
    else if (b.type === 'minutesdoc') {
      out.push({ kind: 'head', text: b.title });
      (b.info || []).forEach(([k, v]) => out.push({ kind: 'body', text: `${k} : ${String(v || '').replace(/\n/g, ' / ')}` }));
      ...
      (b.items || []).forEach((it, n) => {
        out.push({ kind: 'body', text: `${n + 1}) ${it.title}` });
