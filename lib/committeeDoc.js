...
export function rulesPlanBlocks(year, center, members) {
  const y = Number(year);
  const n = {
  ...
  return [
    { type: 'subheading', text: `${y}년도 ${center} 운영위원회 구성 계획` },
    { type: 'para', text: '□ 의의' },
    {
      type: 'para',
...
const PURPOSE =
  '어린이집 운영위원회는 보육 내용과 운영 전반에 관한 사항을 심의하고, 부모와 지역사회의 의견을 어린이집 운영에 반영하기 위하여 구성·운영합니다. ' +
  '우리 어린이집은 원장, 보육교사 대표, 학부모 대표, 지역사회 인사로 운영위원회를 구성하고, 어린이집 회계연도를 기준으로 분기마다 1회씩 연 4회 개최합니다. ' +
  '회의 개최 전에는 일시와 안건을 담은 개최 공지문을 전체 부모에게 미리 알리고, 회의 후에는 논의·결정된 내용을 회의결과 공지문으로 공개하여 ' +
  '운영의 투명성을 높이고 부모가 어린이집 운영에 실질적으로 참여할 수 있도록 하고 있습니다.';

// 그 해의 구성 계획 + 회칙 (원장님이 고친 글이 있으면 그것을 쓴다)
export function rulesBlocks(data, year, center, members) {
  const text = data?.rules?.[year]?.text?.trim() || defaultRulesText(year, center);
  return [
    ...rulesPlanBlocks(year, center, members),
    { type: 'subheading', text: `${year}년 ${center} 운영위원회 회칙` },
    { type: 'para', text },
  ];
}
...
function meetingBlocks(data, i, opts = {}) {
  ...
  if (opts.withRules) {
    b.push({ type: 'subheading', text: `${info.year}년 운영위원 명단` });
    b.push(memberTable(members));
    rulesBlocks(data, info.year, opts.center, members).forEach((x) => b.push(x));
  }

  // ① 개최 공지문
  b.push({ type: 'subheading', text: `${qLabel(i)} 운영위원회 개최 공지문` });
  b.push({ type: 'para', text: m.notice || '' });

  // ② 회의록 (원장님 서식: 표 → 회의순서 → 토의 및 의결사항 → 서명)
  b.push({ type: 'subheading', text: `${qLabel(i)} 운영위원회 회의록` });
  b.push({
    type: 'kv',
    rows: [
      ['일 시', whenText(m)],
  ...
  b.push({ type: 'para', text: `이상으로 ${info.qn}/4분기 어린이집 운영위원회를 마치도록 하겠습니다.` });
  b.push({
    type: 'table',
    head: ['구분', '성명', '서명'],
    widths: ['34%', '33%', '33%'],
  ...
  // ③ 회의결과 공지문
  b.push({ type: 'subheading', text: `${noticeMonthLabel(m, i)} 운영위원회 회의결과 안내문` });
  b.push({ type: 'para', text: m.result || '' });

  if (m.feature) {
    b.push({ type: 'subheading', text: `${info.quarter} 운영의 특징` });
...
export function buildOneMeetingDoc(data, i, basic) {
  ...
  const info = MEETINGS[i];
  return [
    { type: 'title', text: `${center} 운영위원회 ${info.no} (${info.quarter})` },
    ...meetingBlocks(data, i, { withRules: true, center }),
  ];
}
...
export function buildCommitteeDoc(data, basic) {
  const center = basic?.centerName?.trim() || '○○어린이집';
  const blocks = [{ type: 'title', text: `${center} 운영위원회 운영 결과` }];

  blocks.push({ type: 'heading', text: '1. 운영위원회 구성 및 운영 목적' });
  blocks.push({ type: 'para', text: PURPOSE });

  blocks.push({ type: 'heading', text: '2. 운영위원 명단' });
  YEARS.forEach((y) => {
    const list = (data?.members?.[y] || []).filter((x) => x.name?.trim());
    blocks.push({ type: 'subheading', text: `${y}년 운영위원` });
    blocks.push(memberTable(list));
  });

  blocks.push({ type: 'heading', text: '3. 운영위원회 구성 계획 및 회칙' });
  YEARS.forEach((y, n) => {
    if (n) blocks.push({ type: 'divider' });
    const list = (data?.members?.[y] || []).filter((x) => x.name?.trim());
    rulesBlocks(data, y, center, list).forEach((x) => blocks.push(x));
  });

  blocks.push({ type: 'heading', text: '4. 회의 개최 현황' });
  blocks.push({
    type: 'table',
  ...
  blocks.push({ type: 'heading', text: '5. 차수별 개최 내용' });
  let first = true;
