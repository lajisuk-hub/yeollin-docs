...
function meetingBlocks(data, i, opts = {}) {
  ...
  if (opts.withRules) {
    ...
    b.push({ type: 'rulesdoc', text: data?.rules?.[info.year]?.text?.trim() || defaultRulesText(info.year, center) });
...
export function buildOneMeetingDoc(data, i, basic) {
  ...
  if (isFirstOfYear(i)) {
    ...
    blocks.push({ type: 'rulesdoc', text: data?.rules?.[y]?.text?.trim() || defaultRulesText(y, center) });
  } else {
...
export function buildCommitteeDoc(data, basic) {
  ...
  blocks.push({ type: 'heading', text: '1. 회의 개최 현황' });
  blocks.push({
    type: 'table',
  ...
  // 분기별 내용 — 그 해 첫 차수 안에 구성계획·회칙을 함께 넣는다 (연도별 1회)
  blocks.push({ type: 'heading', text: '2. 분기별 운영위 내용' });
  let first = true;
  MEETINGS.forEach((info, i) => {
    if (!meetingHasContent(data.meetings[i])) return;
    if (!first) blocks.push({ type: 'divider' });
    first = false;
    meetingBlocks(data, i, { center, withRules: isFirstOfYear(i) }).forEach((x) => blocks.push(x));
  });
  if (first) {
