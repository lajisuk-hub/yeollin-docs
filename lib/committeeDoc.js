...
function meetingBlocks(data, i, opts = {}) {
  ...
  b.push({
    type: 'noticedoc',
    title: `${qLabel(i)} 어린이집 운영위원회 개최 안내`,
  ...
  b.push({
    type: 'resultdoc',
    title: `${noticeMonthLabel(m, i)} 운영위원회 회의결과 안내문`,
    intro: m.resultIntro || '',
    items: m.resultItems || [],
...
export const isFirstOfYear = (i) => MEETINGS.findIndex((x) => x.year === MEETINGS[i].year) === i;

// 그 해 구성계획·회칙이 어느 차수 문서에 들어 있는지 (안내 문구용)
export const yearDocAt = (i) => MEETINGS[MEETINGS.findIndex((x) => x.year === MEETINGS[i].year)];
