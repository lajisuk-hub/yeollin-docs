...
// 고를 수 있는 학년도 — 2025·2026 (2024는 원장님 지시로 뺌). 다만 이미 그 해로 만들어 둔 자료가 있으면 사라지지 않게 보여 준다
export function tidyYearOptions(data) {
...
export function tidyYearsWithContent(data) {
  return tidyYearOptions(data).filter((y) => {
...
export function chosenTidyMonths(data) {
  // skip — 달 목록에서 「문서에서 빼기」를 누른 달은 자료는 남기고 문서에만 안 넣는다
  return monthList(data.year).filter((x) => monthTidyHasContent(data.months?.[x.key]) && !data.months?.[x.key]?.skip);
}
