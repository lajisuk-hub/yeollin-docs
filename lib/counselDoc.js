...
// 안내문·신청서 배경 그림 (원장님이 준 서식). 다른 그림을 올리면 그것으로 바뀐다.
export const DEFAULT_BG = '/notice-bg.png';
// 예전에 저장해 둔 내용이 이 주소를 가리킬 수 있어 파일은 그대로 둔다(내용은 기본 서식과 같음)
export const DEFAULT_BG_PLAIN = '/notice-bg-plain.png';
export const DEFAULT_APPLY_BG = '/apply-bg.png';
// 2회차는 1회차와 구분되도록 파란색 계열 서식
export const DEFAULT_BG_2 = '/notice-bg-2.png';
export const DEFAULT_APPLY_BG_2 = '/apply-bg-2.png';
// 회차(0=1회차, 1=2회차)에 맞는 기본 서식
export const noticeBgFor = (i) => (i === 1 ? DEFAULT_BG_2 : DEFAULT_BG);
export const applyBgFor = (i) => (i === 1 ? DEFAULT_APPLY_BG_2 : DEFAULT_APPLY_BG);
// 2회차 서식은 아래쪽 그림(잔디·건물)이 더 높아서 글 아래 여백을 넉넉히 둔다
export const noticeBottomFor = (i) => (i === 1 ? 24 : 20);
export const applyBottomFor = (i) => (i === 1 ? 24 : 15);

// 신청서에서 부모가 고를 수 있는 상담 주제
export const DEFAULT_TOPICS = [
  '가정에서의 생활 습관 (수면·식사·배변 등)',
  '또래 관계와 어린이집 적응',
  '발달 상황 (신체·언어·정서 등)',
  '가정에서 특별히 신경 쓰는 부분',
  '그 밖에 담임교사와 나누고 싶은 이야기',
];
...
export const emptyRound = () => ({
  ...
  noticeBg: DEFAULT_BG, noticeTop: 30, noticeBottom: 20, noticeScale: 1.15, noticeAsk: true,
  apply: '', applyFeedback: '', applyIntro: '', applyTopics: '',
  applyBg: DEFAULT_APPLY_BG, applyTop: 27, applyBottom: 15, applyScale: 1.15,
...
export function roundHasContent(r) {
  return !!(r && (r.from || r.notice || r.noticeGreeting || r.apply || r.applyIntro || r.summary || (r.photos || []).length));
}

// 신청서(부모가 작성) 블록 만들기
export function applyBlock(r, i, center) {
  const toList = (s) => String(s || '').split(/\n+/).map((t) => t.trim().replace(/^[·\-*▪]\s*/, '')).filter(Boolean);
  return {
    type: 'apply',
    round: `${i + 1}회차`,
    center,
    bg: r.applyBg === undefined ? DEFAULT_APPLY_BG : r.applyBg,
    top: Number(r.applyTop) || 27,
    bottom: Number(r.applyBottom) || 15,
    textScale: Number(r.applyScale) || 1.15,
    intro: r.applyIntro || r.apply || '',
    period: periodText(r),
    method: r.method || '',
    place: r.place || '',
    topics: toList(r.applyTopics).length ? toList(r.applyTopics) : DEFAULT_TOPICS,
  };
}
...
export function noticeBlock(r, i, center) {
  ...
  return {
    ...
    bottom: Number(r.noticeBottom) || 20,
    textScale: Number(r.noticeScale) || 1.15,
...
function roundBlocks(r, i, center) {
  ...
  // 공지문과 신청서는 나란히 두 칸으로 (문서에서는 크게 볼 필요가 없음)
  b.push({
    type: 'pair',
    labels: ['상담 공지문', '부모상담 신청서'],
    items: [noticeBlock(r, i, center), applyBlock(r, i, center)],
  });

  b.push({ type: 'subheading', text: `${n}회차 상담의 주요 핵심 정리` });
  b.push({ type: 'para', text: r.summary || '' });

  const photos = (r.photos || []).filter(Boolean);
  if (photos.length) {
    b.push({ type: 'photos', items: photos, caption: `${n}회차 상담 사진` });
  }
...
export function buildCounselDoc(data, basic) {
  ...
  blocks.push({ type: 'heading', text: '2. 상담 실시 결과' });
  let first = true;
  rounds.forEach((r, i) => {
    if (!roundHasContent(r)) return;
    // 2회차는 새 쪽으로 넘기지 않고 바로 이어서 (쪽 상단 여백은 인쇄 설정으로 모든 쪽에 들어감)
...
export function toHwpxBlocks(blocks) {
  const out = [];
  // 나란히 둔 서식은 한글에서는 위아래로 풀어서 넣는다
  const flat = blocks.flatMap((b) => (b.type === 'pair' ? b.items : [b]));
  flat.forEach((b) => {
    if (b.type === 'title') out.push({ kind: 'title', text: b.text });
      ...
      out.push({ kind: 'head', text: `${b.round} 부모 개별상담 신청서` });
      if (b.intro) out.push({ kind: 'body', text: b.intro });
      ['반 이름', '영유아 이름', '보호자 성함 (관계)', '연락처'].forEach((k) => out.push({ kind: 'body', text: `${k} : ` }));
      out.push({ kind: 'body', text: '상담 희망 일시 1지망 :        월      일      시      분' });
      out.push({ kind: 'body', text: '상담 희망 일시 2지망 :        월      일      시      분' });
      out.push({ kind: 'body', text: '희망 상담 방법 : □ 대면 상담   □ 전화 상담' });
      out.push({ kind: 'body', text: '[상담에서 나누고 싶은 내용 - 해당하는 곳에 표시해 주세요]' });
      (b.topics || []).forEach((t) => out.push({ kind: 'body', text: `□ ${t}` }));
      out.push({ kind: 'body', text: '더 하고 싶은 이야기 : ' });
      out.push({ kind: 'note', text: b.center });
    }
