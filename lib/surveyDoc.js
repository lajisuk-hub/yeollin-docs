...
// 회신율 = 회신 수 ÷ 배부 부수. 배부 부수를 모르면 부모 총 인원으로 계산한다.
export const replyRate = (d) => {
  const base = num(d?.copies) || num(d?.parents);
  const r = num(d?.replies);
  return base > 0 ? Math.round((r / base) * 100) : 0;
};

// 결과 줄 — 모르는 값(배부 부수 등)은 아예 쓰지 않는다
export function resultCountText(d) {
  const parts = [];
  if (num(d?.parents)) parts.push(`부모 총 ${num(d.parents)}명${d.twins ? ` (${d.twins})` : ''}`);
  if (num(d?.copies)) parts.push(`배부 ${num(d.copies)}부`);
  if (num(d?.replies)) parts.push(`회신 ${num(d.replies)}명${replyRate(d) ? ` (회신율 ${replyRate(d)}%)` : ''}`);
  return parts.join(' / ');
}
...
export function surveyBlocks(data, center, opts = {}) {
  ...
  // 올린 설문지에서 문항을 읽어 온 경우(data.questions)에는 그 문항을 그대로 쓴다.
  const picked = Array.isArray(data.questions) && data.questions.length ? data.questions : null;
  const rows = [];
  AREAS.forEach((a) => {
    const qs = picked
      ? picked.filter((q) => q.area === a.key).map((q) => q.text).filter(Boolean)
      : (QUESTIONS[a.key] || []);
    qs.forEach((q, i) => {
      rows.push([
  ...
  b.push({
    ...
    cls: 'survey',
    head: ['구분', '내용', ...SCALE],
    // 문항 칸은 넓게, 오른쪽 체크 칸은 좁게 (원장님 지시)
    widths: ['12%', '66%', '5.5%', '5.5%', '5.5%', '5.5%'],
    rows,
    leftFirst: true,
  });
  ...
  return [
    { type: 'greenbar', text: `${y}년 ${center} 부모만족도 조사결과 안내` },
    {
      ...
      items: [
        ...
        { label: '결과', value: resultCountText(data) },
