...
export const DOCS = [
  ...
  // ───────────────────────── 참여성 ─────────────────────────
  {
    ...
    fields: [
      ...
      { type: 'section', label: '［2회차 · 하반기］  (아직이면 비워두세요)' },
      { key: 's2notice', label: '① 상담 공지문 (사진 또는 PDF)', type: 'attach' },
      { key: 's2apply', label: '② 부모상담 신청서 (사진 또는 PDF)', type: 'attach' },
      { key: 's2period', label: '③ 시기', type: 'text', placeholder: '예) 2026. 11. 11.~11. 15. (하반기)' },
      { key: 's2count', label: '④ 참여 인원', type: 'text', placeholder: '예) 전체 40명 중 39명 참여' },
      { key: 's2material', label: '⑤ 상담한 자료 (문서 업로드 또는 붙여넣기 → AI 분석)', type: 'material', placeholder: '2회차(하반기) 상담 자료를 올리거나 붙여넣으세요. 아직 안 했으면 비워두세요.' },
    ...
    build: (v, ai) => {
      ...
      const session = (label, notice, apply, period, count, summary, topics, material, photos, cap) => {
        out.push({ type: 'subheading', text: label });
        out.push({ type: 'para', text: '· 상담 공지문' });
        out.push({ type: 'pages', items: notice, emptyText: '※ 공지문 사진 또는 PDF를 첨부해 주세요.' });
        out.push({ type: 'para', text: '· 부모상담 신청서' });
        out.push({ type: 'pages', items: apply, emptyText: '※ 신청서 사진 또는 PDF를 첨부해 주세요.' });
        out.push({ type: 'kv', rows: [['시기', period], ['참여 인원', count]] });
        if (summary || material) {
          out.push({ type: 'para', text: '· 상담 내용 분석' });
          out.push({ type: 'para', text: summary || '' });
          out.push({ type: 'para', text: topics || material || '' });
        }
        if (Array.isArray(photos) && photos.length) {
          out.push({ type: 'images', items: photos, caption: cap });
        }
      };

      session('［1회차 · 상반기］', v.s1notice, v.s1apply, v.s1period, v.s1count, ai?.s1summary, ai?.s1topics, v.s1material, v.s1photos, '1회차 상담 사진');
      ...
      if (has2) {
        session('［2회차 · 하반기］', v.s2notice, v.s2apply, v.s2period, v.s2count, ai?.s2summary, ai?.s2topics, v.s2material, v.s2photos, '2회차 상담 사진');
      }

      out.push({ type: 'note', text: '※ 본 문서는 부모 개별상담(연 2회, 상·하반기 각 1회) 운영 결과 증빙자료입니다. 상담 원자료는 어린이집에 별도 보관합니다.' });
