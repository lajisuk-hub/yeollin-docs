...
export const DOCS = [
  ...
  // ───────────────────────── 참여성 ─────────────────────────
  {
    ...
    item: '2-가. 부모 개별상담 (5점)',
    desc: '부모상담 목적·공지문·일정·신청서와, 상·하반기 상담 실시 결과(참여인원·상담내용)를 한 문서로 정리합니다. 상담한 내용을 붙여넣으면 AI가 총 참여와 주요 상담내용을 분석해 드립니다.',
    fields: [
      ...commonHead,
      { key: 'year', label: '연도', type: 'text', placeholder: '예) 2026' },
      { key: 'purposeMemo', label: '부모상담을 하는 이유 (간단 메모 · 비워두면 AI가 표준 문구로 채움)', type: 'textarea', placeholder: '예) 영유아의 발달과 생활 전반을 부모와 공유하고 가정과의 연계를 강화하기 위해 정기 상담을 운영함' },
      { key: 'noticeImg', label: '① 부모상담 공지문 이미지', type: 'image' },
      { key: 'applyImg', label: '② 부모상담 신청서 이미지', type: 'image' },
      { key: 'schedule', label: '③ 부모상담 일정', type: 'textarea', placeholder: '예) 상반기 6/10~6/14, 하반기 11/11~11/15 / 반별 시간표 등' },
      { key: 's1period', label: '[1회차] 시기', type: 'text', placeholder: '예) 2026. 6. 10.~6. 14. (상반기)' },
      { key: 's1count', label: '[1회차] 참여 인원', type: 'text', placeholder: '예) 전체 40명 중 38명 참여' },
      { key: 's1material', label: '[1회차] 상담한 내용 붙여넣기 (AI가 분석)', type: 'textarea', placeholder: '상담한 내용을 그대로 붙여넣으세요. 여러 아이 것을 이어붙여도 됩니다. AI가 주요 상담내용을 한눈에 정리합니다.' },
      { key: 's1photos', label: '[1회차] 상담 사진 (여러 장 가능)', type: 'images' },
      { key: 's2period', label: '[2회차] 시기 (아직이면 비워두세요)', type: 'text', placeholder: '예) 2026. 11. 11.~11. 15. (하반기)' },
      { key: 's2count', label: '[2회차] 참여 인원', type: 'text', placeholder: '예) 전체 40명 중 39명 참여' },
      { key: 's2material', label: '[2회차] 상담한 내용 붙여넣기 (AI가 분석)', type: 'textarea', placeholder: '2회차(하반기) 상담 내용을 붙여넣으세요. 아직 안 했으면 비워두세요.' },
      { key: 's2photos', label: '[2회차] 상담 사진 (여러 장 가능)', type: 'images' },
    ],
    ai: {
      button: 'AI로 상담내용 분석·정리',
      keys: ['purpose', 's1summary', 's1topics', 's2summary', 's2topics'],
      system: '당신은 어린이집 부모 개별상담 운영 결과를 정리하는 전문가입니다. 원장이 붙여넣은 상담 원자료를 분석해 공식 문서 문어체로 정리합니다. 없는 사실은 지어내지 않으며, 원자료가 비어 있는 회차의 키는 빈 문자열("")로 둡니다. 큰따옴표(")는 절대 쓰지 않습니다.\n출력 키 설명:\n- purpose: 부모상담을 운영하는 목적을 3~4문장으로. 원장 메모가 있으면 그것을 다듬고, 없으면 영유아 발달·생활 공유와 가정연계 강화라는 표준 취지로 작성.\n- s1summary: 1회차 종합 정리 2~3문장(참여 규모와 전반적 상담 분위기 중심).\n- s1topics: 1회차 주요 상담 내용을 주제별로 묶어 한눈에 보이게 정리. 각 줄을 · 로 시작하고 줄바꿈(\\n)으로 구분. 예: 발달·놀이, 건강·수면·식사, 또래관계, 가정연계 요청 등.\n- s2summary, s2topics: 2회차에 대해 동일한 방식. 2회차 원자료가 없으면 둘 다 "".\n아래 JSON 하나만 출력합니다. {"purpose":"","s1summary":"","s1topics":"","s2summary":"","s2topics":""}',
      user: (v) => `어린이집: ${v.centerName || ''} (${v.year || ''}년)\n부모상담 목적 메모: ${v.purposeMemo || '(없음 — 표준 취지로 작성)'}\n\n[1회차] 시기: ${v.s1period || ''} / 참여: ${v.s1count || ''}\n[1회차] 상담 원자료:\n${v.s1material || '(없음)'}\n\n[2회차] 시기: ${v.s2period || ''} / 참여: ${v.s2count || ''}\n[2회차] 상담 원자료:\n${v.s2material || '(없음)'}`,
    },
    build: (v, ai) => {
      const out = [];
      out.push({ type: 'title', text: `${v.year ? v.year + '년 ' : ''}부모 개별상담 운영 결과` });
      out.push({ type: 'kv', rows: [['어린이집', v.centerName], ['연도', v.year], ['상담 횟수', '연 2회 (상·하반기 각 1회)']] });

      out.push({ type: 'heading', text: '1. 부모상담 운영 목적' });
      out.push({ type: 'para', text: ai?.purpose || v.purposeMemo || '영유아의 발달과 생활 전반을 부모와 공유하고, 가정과 어린이집의 연계를 강화하기 위하여 상·하반기 각 1회 부모 개별상담을 운영합니다.' });

      out.push({ type: 'heading', text: '2. 부모상담 공지문' });
      out.push({ type: 'image', src: v.noticeImg, emptyText: '※ 공지문 이미지를 첨부해 주세요.' });

      out.push({ type: 'heading', text: '3. 부모상담 일정' });
      out.push({ type: 'para', text: v.schedule || '' });

      out.push({ type: 'heading', text: '4. 부모상담 신청서' });
      out.push({ type: 'image', src: v.applyImg, emptyText: '※ 신청서 이미지를 첨부해 주세요.' });

      out.push({ type: 'heading', text: '5. 상담 실시 결과' });

      // 1회차
      out.push({ type: 'subheading', text: '［1회차 · 상반기］' });
      out.push({ type: 'kv', rows: [['시기', v.s1period], ['참여 인원', v.s1count]] });
      if (ai?.s1summary || v.s1material) {
        out.push({ type: 'para', text: ai?.s1summary || '' });
        out.push({ type: 'subheading', text: '주요 상담 내용' });
        out.push({ type: 'para', text: ai?.s1topics || v.s1material || '' });
      }
      if (Array.isArray(v.s1photos) && v.s1photos.length) {
        out.push({ type: 'images', items: v.s1photos, caption: '1회차 상담 사진' });
      }

      // 2회차 (있을 때만)
      const has2 = v.s2period || v.s2count || v.s2material || (Array.isArray(v.s2photos) && v.s2photos.length) || ai?.s2summary;
      if (has2) {
        out.push({ type: 'subheading', text: '［2회차 · 하반기］' });
        out.push({ type: 'kv', rows: [['시기', v.s2period], ['참여 인원', v.s2count]] });
        if (ai?.s2summary || v.s2material) {
          out.push({ type: 'para', text: ai?.s2summary || '' });
          out.push({ type: 'subheading', text: '주요 상담 내용' });
          out.push({ type: 'para', text: ai?.s2topics || v.s2material || '' });
        }
        if (Array.isArray(v.s2photos) && v.s2photos.length) {
          out.push({ type: 'images', items: v.s2photos, caption: '2회차 상담 사진' });
        }
      }

      out.push({ type: 'note', text: '※ 본 문서는 부모 개별상담(연 2회, 상·하반기 각 1회) 운영 결과 증빙자료입니다. 상담 원자료·신청서는 어린이집에 별도 보관합니다.' });
      out.push({ type: 'sign', text: v.centerName, role: '원장', name: '' });
      return out;
    },
  },
  {
