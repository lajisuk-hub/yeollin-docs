...
export const DOCS = [
  ...
  // ───────────────────────── 참여성 ─────────────────────────
  {
    ...
    fields: [
      ...
      { key: 's1period', label: '③ 시기', type: 'text', placeholder: '예) 2025년 10월' },
      ...
      { key: 's2period', label: '③ 시기', type: 'text', placeholder: '예) 2026년 3월' },
    ...
    build: (v, ai) => {
      ...
      const session = (no, notice, apply, period, count, summary, topics, material, photos, cap, brk) => {
        if (brk) out.push({ type: 'pagebreak' });
        out.push({ type: 'subheading', text: period ? `${no} : ${period}` : no });
