...
import { buildProgramTidyDoc, tidyYearsWithContent } from './programTidyDoc';
...
export const SECTIONS = [
  ...
  { key: 'program', store: 'program-tidy', docId: 'program-tidy', area: '참여성', name: '부모참여프로그램', build: (d, b) => buildProgramTidyDoc(d, b, 'all'), has: (d) => !!tidyYearsWithContent(d || {}).length },
...
const softenBreaks = (blocks) => blocks.map((b) => (b.type === 'pagebreak' && !b.hard ? { type: 'divider' } : b));
