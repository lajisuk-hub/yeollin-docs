...
export function monthSetBlocks(data, mi, center, opts = {}) {
  ...
  if (opts.compact) {
    b.push({
      type: 'pair',
      compact: true,
      labels: ['① 부모 안내문 (공지문)', '② 실시기록 (결과보고서)'],
      items: [noticeBlock(data, mi, center), recordBlock(data, mi, center)],
    });
  } else {
  ...
  if (photos.length) {
    b.push({ type: 'photos', items: photos, caption: `${mi.label} 부모참여프로그램 활동 사진`, small: !!opts.compact });
  }
  return b;
}
...
export function buildProgramDoc(data, basic, rangeKey = 'all') {
  ...
  // 전체 문서에서는 달마다 공지문·실시기록을 좌우 2단으로 작게 (한 달이 한 쪽에 들어가게)
  picks.forEach((mi, i) => {
