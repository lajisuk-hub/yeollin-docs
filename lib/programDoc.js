import { blocksToHwpx } from './hwpxBlocks';
...
export function chosenMonths(data) {
  ...
  // skip — 달 목록에서 「문서에서 빼기」를 누른 달 (자료는 남기고 문서에만 안 넣는다)
  const skip = (x) => !!data.months?.[x.key]?.skip;
  if (data.mode === 'quarter') return list.filter((x) => (data.picked || []).includes(x.key) && !skip(x));
  return list.filter((x) => monthHasContent(data.months?.[x.key]) && !skip(x));
}

// ── 여러 학년도 (2026-09-19) ──
// 심사 기간이 두 학년도에 걸치므로, 작년 자료를 만든 뒤 학년도를 바꿔 올해 자료를 이어서 만들 수 있다.
//   · 달별 자료(months)는 열쇠가 '2025-03'처럼 해를 품고 있어 한 곳에 같이 둔다
//   · 연간계획·제출 방식(분기/월)·고른 달은 해마다 다르므로, 지금 보고 있지 않은 해의 것은 data.archive[해]에 보관한다
//   · 연령·참여 대상·필요성·서식은 두 해가 함께 쓴다
const YEAR_FIELDS = ['plan', 'planFeedback', 'mode', 'picked'];
const emptyYearPart = () => ({ plan: [], planFeedback: '', mode: '', picked: [] });

export function programYearOptions(data) {
  const set = new Set(['2025', '2026', String(data?.year || '2026'), ...Object.keys(data?.archive || {})]);
  return Array.from(set).filter((y) => /^\d{4}$/.test(y)).sort();
}

export function switchProgramYear(d, y) {
  const cur = String(d.year || '2026');
  if (y === cur) return d;
  const keep = {};
  YEAR_FIELDS.forEach((k) => { keep[k] = d[k]; });
  const archive = { ...(d.archive || {}), [cur]: keep };
  const load = archive[y] || emptyYearPart();
  delete archive[y];
  return { ...d, ...emptyYearPart(), ...load, archive, year: y };
}

export function programYearView(data, y) {
  if (String(y) === String(data.year || '2026')) return data;
  return { ...data, ...emptyYearPart(), ...(data.archive?.[y] || {}), year: y };
}

// 그 해에 만든 달 수 (내용이 있는 달)
export const programYearCount = (data, y) =>
  monthList(y).filter((x) => monthHasContent(data.months?.[x.key])).length;

// 자료가 들어 있는 학년도 (앞선 해부터)
export function programYearsWithContent(data) {
  return programYearOptions(data).filter((y) => {
    const v = programYearView(data, y);
    return (v.plan || []).some((p) => p.theme) || programYearCount(data, y) > 0;
  });
}
...
// narrow — 전체 문서에서는 그림을 조금 좁혀 앞 내용에 이어 붙게 한다
//          (그림이 쪽 높이를 거의 다 차지하면 통째로 다음 쪽으로 밀려 앞 쪽이 텅 빈다)
export function noticeBlock(data, mi, center, narrow = false) {
...
// opts.compact — 전체 문서에서는 공지문과 실시기록을 좌우 2단으로 작게 넣어 쪽수를 줄인다 (원장님 지시)
export function monthSetBlocks(data, mi, center, opts = {}) {
  ...
  b.push(noticeBlock(data, mi, center, !!opts.compact));
...
// 두 학년도에 자료가 있으면: 1. 필요성 → [앞선 해: 2·3] → (새 쪽) [다음 해: 4·5]
export function buildProgramDoc(data, basic, rangeKey = 'all') {
  const center = basic?.centerName?.trim() || '○○어린이집';
  const r = rangeInfo(rangeKey);
  // 분기만 뽑아 저장할 때는 지금 보고 있는 해만
  const found = r.months ? [] : programYearsWithContent(data);
  const years = found.length ? found : [String(data.year || '2026')];
  ...
  years.forEach((y, yi) => {
    ...
    blocks.push({ type: 'heading', text: `${no}. ${multi ? `${y}년도 ` : ''}${r.months ? `${r.label} ` : ''}월별 운영 자료 (공지문 · 결과보고서)` });
    no += 1;
    ...
    if (!picks.length) {
      blocks.push({
        ...
        text: r.months
          ...
          : `※ ${multi ? `${y}년도에 ` : ''}아직 작성한 달이 없습니다. 연간계획을 세운 뒤 달을 골라 공지문과 실시기록을 만들어 주세요.`,
      });
      return;
    }
    // 전체 문서는 '중간 공백보다 이어짐'이 중요하다는 원장님 지시에 따라
    // 달마다 새 쪽에서 시작하지 않고 점선으로 구분하며 그대로 이어 붙인다.
    picks.forEach((mi, i) => {
      if (i > 0) blocks.push({ type: 'divider' });
      monthSetBlocks(v, mi, center, { compact: true }).forEach((x) => blocks.push(x));
    });
  });
  return blocks;
}
...
// 화면 블록 → 한글(hwpx) — 표·사진·쪽 나눔을 그대로 넣는 공통 변환기 (lib/hwpxBlocks.js, 2026-09-19)
export function toHwpxBlocks(blocks) {
  return blocksToHwpx(blocks);
}
