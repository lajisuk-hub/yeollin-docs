...
export function attendText(x) {
  const parts = [];
  if (x?.parents) parts.push(`부모 ${x.parents}명`);
  if (x?.kids) parts.push(`영유아 ${x.kids}명`);
  if (x?.staff) parts.push(`교직원 ${x.staff}명`);
  return parts.join(' · ');
}

export function totalCount(x) {
  const n = (v) => Number(String(v || '').replace(/[^0-9]/g, '')) || 0;
  return n(x?.parents) + n(x?.kids) + n(x?.staff);
}
...
function recordBlock(data, mi, center) {
  ...
  return {
    ...
    summaryTitle: '운영 내용 정리',
    summary: [x.summary, x.review].filter(Boolean).join('\n\n'),
    center,
  };
}
...
export function buildProgramDoc(data, basic) {
  const center = basic?.centerName?.trim() || '○○어린이집';
  const y = data.year || '2026';
  const picks = chosenMonths(data);
  const blocks = [
    { type: 'title', text: `${center}의 부모참여프로그램` },
    { type: 'lead', text: `${y}년 3월 ~ ${Number(y) + 1}년 2월` },
    ...
    { type: 'heading', text: '3. 월별 운영 자료 (공지문 · 결과보고서)' },
  ];

  if (!picks.length) {
    blocks.push({ type: 'note', text: '※ 아직 작성한 달이 없습니다. 연간계획을 세운 뒤 달을 골라 공지문과 실시기록을 만들어 주세요.' });
    return blocks;
  }
...
export function toHwpxBlocks(blocks) {
  ...
  blocks.forEach((b) => {
    if (b.type === 'title') out.push({ kind: 'title', text: b.text });
      ...
      (b.rows || []).forEach((r) => out.push({ kind: 'body', text: r.join(' | ') }));
      if (b.summary) {
        out.push({ kind: 'head', text: b.summaryTitle || '운영 내용 정리' });
        out.push({ kind: 'body', text: b.summary });
      }
    }
