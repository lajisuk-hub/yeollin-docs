...
export function frontBlocks(all, center, docs) {
  ...
  return [
    ...
    // 운영 목표 다음에 연간일정을 이어 붙인다 (쪽을 나누면 앞 쪽이 많이 빈다)
...
export function buildAllDoc(all, basic, docs) {
  ...
  // 서류 사이는 쪽을 나누지 않고 이어 붙인다 (중간 공백보다 이어짐 우선 — 원장님 지시).
  // 대신 영역 구분 줄이 굵은 선으로 구분해 주고, 인쇄에서 위 여백을 넉넉히 둔다.
  let n = 0;
  SECTIONS.forEach((s) => {
    const d = docs[s.key];
    ...
    s.build(d, basic).forEach((b) => {
      ...
      first = false;
      // 한 권으로 묶을 때는 서류 안의 쪽 나눔도 점선 구분으로 바꿔 이어 붙인다
      // (따로 저장할 때는 원래대로 새 쪽에서 시작한다)
