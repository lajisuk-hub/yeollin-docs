...
export function frontBlocks(all, center, docs) {
  ...
  return [
    ...
    // 운영 목표와 연간일정은 내용이 다르므로 쪽을 나눈다 (원장님 지시 2026-08-17)
    { type: 'pagebreak' },
...
export function buildAllDoc(all, basic, docs) {
  ...
  // 내용이 달라지는 자리는 아래가 비더라도 새 쪽에서 시작한다 (원장님 지시 2026-08-17).
  //  · 서류가 바뀌는 자리 (1. 부모 개별상담 / 2. 운영위원회 …)
  //  · 서류 안의 번호 항목이 바뀌는 자리 (1. 필요성 / 2. 공지문 / 3. 설문지 …)
  //  · 서류가 원래 가지고 있던 쪽 나눔 (차수·신청서 등)
  // 한 회차·한 달 안에서는 예전처럼 이어 붙인다 (여기까지 나누면 빈 쪽이 너무 많아짐).
  // 지금 쪽에 아직 알맹이가 없으면(구분 줄·제목·한 줄 안내뿐이면) 또 나누지 않는다 — 제목만 있는 빈 쪽 방지
  ...
  const numbered = (t) => /^\s*\d+\s*[.)]/.test(String(t || ''));
  ...
  SECTIONS.forEach((s) => {
    ...
    s.build(d, basic).forEach((b) => {
      ...
      if (b.type === 'title') { breakPage(); blocks.push({ type: 'heading', text: b.text }); return; }
      first = false;
      if (b.type === 'pagebreak') { breakPage(); return; }
      // 「1. …」「2. …」처럼 번호가 붙은 큰 항목은 새 쪽에서 시작
      if (b.type === 'heading' && numbered(b.text)) { breakPage(); blocks.push(b); return; }
