...
import { buildCommitteeDoc, meetingsOf, whenText as cmWhen, meetingHasContent } from './committeeDoc';
import { buildProgramDoc, monthList, monthOf, planOf, monthHasContent, whenText as pgWhen, programYearsWithContent, programYearView } from './programDoc';
...
export const SECTIONS = [
  ...
  // 지자체 자체기준 — 두 길이 같은 화면(LocalTidy)을 쓰되 저장 자리는 다르다 (local-new / local-tidy)
  { key: 'local', store: 'local-new', area: '지자체', name: '지자체 자체기준', pt: 15, build: buildLocalTidyDoc },
];
...
export function sectionHasData(key, d) {
  ...
  if (key === 'program') return Object.values(d.months || {}).some((x) => monthHasContent(x) && !x.skip);
  ...
  if (key === 'local') return localHas(d);
  return false;
}
...
export function scheduleRows(all, docs) {
  ...
  if (cm) {
    const cmList = meetingsOf(cm);
    (cm.meetings || []).forEach((m, i) => {
      if (!meetingHasContent(m) || !cmList[i]) return;
      push('참여성', `운영위원회 ${cmList[i].no} (${cmList[i].quarter})`, cmWhen(m) || cmList[i].when, '');
    });
  }
  const pg = docs.program;
  if (pg) {
    // 두 학년도에 걸쳐 만들었으면 앞선 해부터 모두
    const pgYears = programYearsWithContent(pg);
    (pgYears.length ? pgYears : [pg.year || all.year]).forEach((y) => {
      const v = programYearView(pg, y);
      monthList(y).forEach((mi) => {
        const x = pg.months?.[mi.key];
        if (!monthHasContent(x) || x.skip) return;
        push('참여성', `부모참여프로그램 - ${planOf(v, mi.m).theme || mi.label}`, pgWhen(x) || mi.label, '');
      });
    });
  }
...
export function frontBlocks(all, center, docs) {
  ...
  return [
    ...
    // 운영 목표와 연간일정은 이어 붙여도 되는 부분 (원장님 지시 2026-08-17)
    { type: 'heading', text: `${y}년도 열린어린이집 연간일정` },
...
export function buildAllDoc(all, basic, docs) {
  ...
  // 쪽을 나누는 자리는 '큰 단락'뿐이다 — 서류가 바뀌는 자리 (원장님 지시 2026-08-17).
  //   예) [참여성] 1. 부모 개별상담 → [참여성] 2. 어린이집 운영위원회 → …
  // 서류 안의 작은 번호(1. 필요성 / 2. 공지문 …)나 회차·달·활동은 나누지 않고 이어 붙인다.
  // 지금 쪽에 아직 알맹이가 없으면(제목·한 줄 안내뿐이면) 또 나누지 않는다 — 제목만 있는 빈 쪽 방지
  const HEAD_ONLY = ['sectionhead', 'heading', 'title', 'lead', 'pagebreak'];
  const breakPage = () => {
    if (!blocks.length) return;
    for (let i = blocks.length - 1; i >= 0; i -= 1) {
      const t = blocks[i].type;
      if (t === 'pagebreak') return;          // 제목뿐인 쪽 — 나누지 않는다
      if (!HEAD_ONLY.includes(t)) break;      // 알맹이가 있으니 나눈다
    }
  ...
  SECTIONS.forEach((s) => {
    ...
    s.build(d, basic).forEach((b) => {
      // 각 서류의 맨 앞 제목은 바로 위 구분 줄과 겹치므로 넣지 않는다
      if (b.type === 'title' && first) { first = false; return; }
      if (b.type === 'title') { blocks.push({ type: 'heading', text: b.text }); return; }
      first = false;
      // 서류 안의 쪽 나눔은 점선 구분으로 바꿔 이어 붙인다 (따로 저장할 때는 원래대로 새 쪽)
      if (b.type === 'pagebreak') { blocks.push(b.hard ? b : { type: 'divider' }); return; }
      // 한 권으로 묶을 때는 상자들을 조금 촘촘하게 (쪽 공백 최소화)
      if (['noticedoc', 'recorddoc', 'minutesdoc', 'resultdoc'].includes(b.type)) {
        blocks.push({ ...b, tight: true });
        return;
      }
      blocks.push(b);
    });
